...
export async function seedDatabase() {
  ...
  let existing = await db.select().from(federations);
  let zrf: typeof federations.$inferSelect;
  let sharks: typeof clubs.$inferSelect;
  let stoneTown: typeof clubs.$inferSelect;
  let pemba: typeof clubs.$inferSelect;

  if (existing.length > 0) {
    zrf = existing[0];
    const allClubs = await db.select().from(clubs);
    sharks = allClubs.find(c => c.name === "Sharks RFC")!;
    stoneTown = allClubs.find(c => c.name === "Stone Town RFC")!;
    pemba = allClubs.find(c => c.name === "Pemba RFC")!;
  } else {
    const [created] = await db.insert(federations).values({
      name: "Zanzibar Rugby Federation", country: "Tanzania", logo: null,
    }).returning();
    zrf = created;

    const [s] = await db.insert(clubs).values({
      federationId: zrf.id, name: "Sharks RFC", location: "Stone Town, Zanzibar",
    ...
    const [st] = await db.insert(clubs).values({
      federationId: zrf.id, name: "Stone Town RFC", location: "Shangani, Stone Town",
    ...
    const [p] = await db.insert(clubs).values({
      federationId: zrf.id, name: "Pemba RFC", location: "Chake Chake, Pemba",
  ...
  // Only create memberships, events, activities if this is a fresh seed
  ...
  if (existingMemberships.length === 0) {
    ...
    await db.insert(events).values([
      { clubId: sharks.id, federationId: zrf.id, title: "Sharks Weekly Training", type: "training", date: fmt(tomorrow), time: "17:00", location: "Mnazi Mmoja Grounds, Stone Town", description: "Regular training session. All levels welcome." },
      { clubId: stoneTown.id, federationId: zrf.id, title: "Touch Rugby Afternoon", type: "touch_rugby", date: fmt(nextSat), time: "15:00", location: "Forodhani Gardens", description: "Casual touch rugby session. Open to everyone!" },
      { clubId: sharks.id, federationId: zrf.id, title: "Sharks vs Stone Town", type: "match", date: fmt(nextWeek), time: "14:00", location: "Amaan Stadium", description: "Friendly match between Sharks RFC and Stone Town RFC." },
      { clubId: pemba.id, federationId: zrf.id, title: "Pemba Fitness Camp", type: "training", date: fmt(nextSat), time: "08:00", location: "Chake Chake Stadium", description: "Intensive fitness and conditioning camp." },
      { clubId: sharks.id, federationId: zrf.id, title: "ZRF Season Opening Tournament", type: "tournament", date: fmt(new Date(today.getFullYear(), today.getMonth() + 1, 15)), time: "09:00", location: "Amaan Stadium", description: "Official start of the new rugby season." },
    ]);
